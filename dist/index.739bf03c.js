...
function toggleGenres(e) {
    if (e.currentTarget.checked === true) state.selectedGenre.push(e.currentTarget.value);
    else state.selectedGenre = state.selectedGenre.filter((genre)=>genre !== e.currentTarget.value
    );
    render();
}
function toggleYears(e) {
    if (e.currentTarget.checked === true) state.selectedYear.push(e.currentTarget.value);
    else state.selectedYear = state.selectedYear.filter((year)=>year !== e.currentTarget.value
    );
    render();
}
function render() {
    ...
    if (state.mediaType !== '') items = items.filter((item)=>item.type === state.mediaType
    );
    if (state.selectedGenre.length > 0) items = items.filter((item)=>item.genre.some((genre)=>state.selectedGenre.includes(genre)
        )
    );
    if (state.selectedYear.length > 0) items = items.filter((item)=>state.selectedYear.includes(item.year)
    );
    if (state.search !== '') items = items.filter((item)=>item.title.toLowerCase().indexOf(state.search.toLowerCase()) >= 0
    );
    items.map((media)=>{
        const item = `
            <div class="results-item">
                ...
                <h3>${media.title}</h3>
                <h4>Genres: ${media.genre.map((genre)=>" " + genre[0].toUpperCase() + genre.substring(1)
        ).toString()}</h4>
            </div>
        `;
    ...
    Array.from(document.getElementsByClassName('filter-bar-filters-filter')).forEach((el)=>{
        ...
        if (state[key]) document.getElementById(el.id + "-list").classList = "filter-bar-filters-list open";
        else document.getElementById(el.id + "-list").classList = "filter-bar-filters-list";
    });
...
function main() {
    fetch('https://raw.githubusercontent.com/HubSpotWebTeam/CodeExercise/main/src/js/data/data.json').then((res)=>res.json()
    ).then((data)=>{
        ...
        state.genres = state.genres.filter((value, index, self)=>self.indexOf(value) === index
        ).sort();
        state.years = state.years.filter((value, index, self)=>self.indexOf(value) === index
        ).sort((a, b)=>a - b
        );
        render();
...
},{"../styles/index.css":"3frGm"}],"3frGm":[function() {},{}]},["l4AUa","ebWYT"], "ebWYT", "parcelRequirec6bf")
